...
switch strtrim(hostname)
    case {'ATWSN647','schleppi'}
        logfile_directory = '';
    otherwise
        logfile_directory = 'E:\auditory-objects\benware\benware\astrid\';
end
...
vowel_level     = 85;                   % vowel level in dB (relates to mean level of DRCs)(masking in noise see Bizley et al. 2013 JASA) chose 4dB below overall level of DRC (result of worst ferret)
vowel_position  = [3 7 30];             % position of the vowel after first switch in background DRC (chord number after switch) [3 7 16]
...
n_chord        = 60;                   % number of chords, n_chord*chord_duration should equal about 4 sec overall stimulus length per jitter condition (Dahmen et al. 10 used 5s but said that for adaptation only a few 100ms are sufficient)
jitter         = [0.001 0.001; 0.001 0.04; 0.04 0.001; 0.04 0.04];       % frequency jitter in percent of frequency
levels_offset  = 60;                   % mean level for each frequency component, e.g. 45 offset plus 10 range = mean of 50 dB with range [45,55] (=0..10+45) dB, overall level 50 dB, 100 components is about 71dB
