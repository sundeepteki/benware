function computer_name = get_current_computer_name
  ...
  % cluster_root
    if isunix
      try
        [status result] = system('/sbin/ifconfig | grep 00:30:48:57:3C:90 | wc -l');
        if str2num(result(1))
          computer_name = 'cluster_root';
        end
      catch
      end
    end
  ...
  % percheron
  try
      [status result] = system('/sbin/ifconfig | grep 00:12:3f:7c:7e:4b | wc -l');
      if str2num(result(1))
          computer_name = 'percheron';
      end
  catch
  end
