function new_contrast_version
  
  % get current version
    v.str = get_contrast_version();
  ...
  switch todo
	...
	for ii=1:L(subdirs)
      ...
      % make new subdir
        eval(['!mkdir old.disabled/lib/v' vnew.sedstr]);
      % copy contents
        eval(['!cp lib/*.m old.disabled/lib/v' vnew.sedstr]);
        eval(['!cp *.m old.disabled/lib/v' vnew.sedstr]);
        subdirs = dir('lib/*');
        subdirs = subdirs([subdirs.isdir]);
        subdirs = subdirs(3:end);
        for ii=1:L(subdirs)
          switch subdirs(ii).name
            case {'old.disabled','v1.1'}
              continue;
            otherwise
              eval(['!cp -R lib/' subdirs(ii).name ' old.disabled/lib/v' vnew.sedstr '/']);
          end
        end
  end      
