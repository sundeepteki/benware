function stim_ds = downsample_stim_circular(stim,block_size_ms,sample_rate)
  % DOWNSAMPLE_STIM_CIRCULAR
  %   stim_ds = downsample_stim_circular(stim,block_size_ms,sample_rate)
  % 
...
% ================
  ...
  if any(stim(:)<0)
    error('input:error','stimulus cannot be negative, in order for integration to work');
  end
...
%% subdivisions (ie "blocks")
  ...
  for ii=1:block.n
    ...
    ti = block.boundary(1);
    tf = block.boundary(2);
    try stim_ds(:,ii) = circ_mean( stim(:,ti:tf) );
    catch
      keyboard;
    end
  end
