function stimDeviceInit(deviceName, sampleRateHz)
% stimDeviceInit(deviceName, sampleRateHz)

global stimDevice;

if sampleRateHz>40000 && sampleRateHz<=50000
    sampleRate = 3;
elseif sampleRateHz>90000 && sampleRateHz<=100000
    sampleRate = 4;
else
...
if ~isempty(stimDevice)
    ...
    if stimDevice.GetSFreq==sampleRateHz
        fprintf('  * %s already initialised at correct sample rate, doing nothing\n', deviceName);
    else
