...
figure(3);
collapsed_data_freqs = squeeze(sum(sum(spikes,2), 3)); % sum over freq, level
for chan_idx   = 1:n_chans
  subplot(ceil(n_chans/4), 4, plot_order(chan_idx));
  errorbar(freq_idx, mean(spikes(:,chan_idx)), std(spikes(:,chan_idx))/sqrt(length(spikes(:,chan_idx))) );
