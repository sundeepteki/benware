function grid = grid_st_sequencelearning_A_2()
...
if(strcmpi(grid.animalgroup,'Experimental') && strcmpi(grid.condition,'Test'))
    ...
    vowel.sequence                  = {'ABCD' 'DCBA' 'ABCD300' 'ABCDnew'};
    vowel.repeats                   = 48;
    vowel.sequence_id               = repmat(vowel.sequence,1,vowel.repeats);
        vowel.rand_sequence_id      = tshuffle([1*ones(1,vowel.repeats) 2*ones(1,vowel.repeats) 3*ones(1,vowel.repeats) 4*ones(1,vowel.repeats)]);
